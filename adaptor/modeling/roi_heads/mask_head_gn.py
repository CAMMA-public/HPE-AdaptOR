...
# '''
# modified from https://github.com/facebookresearch/detectron2/blob/master/detectron2/modeling/roi_heads/mask_head.py
...
def mask_rcnn_loss(
    ...
    for instances_per_image in instances:
        ...
        if "BitMasks" in str(
            type(instances_per_image.gt_masks)
        ) or "PolygonMasks" in str(type(instances_per_image.gt_masks)):
            gt_masks_per_image = instances_per_image.gt_masks.crop_and_resize(
                instances_per_image.proposal_boxes.tensor, mask_side_len
            ).to(device=pred_mask_logits.device)
        else:
            gt_masks_per_image = instances_per_image.gt_masks.to(
...
class MaskRCNNConvUpsampleHeadGN(BaseMaskRCNNHead, nn.Sequential):
    """
